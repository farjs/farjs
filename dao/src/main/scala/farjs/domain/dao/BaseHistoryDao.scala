...
abstract class BaseHistoryDao(val ctx: FarjsDBContext,
                              tableName: String,
                              maxItemsCount: Int) {

  import ctx._

  def getAll: Future[Seq[BaseHistory]] = {
    ctx.performIO(ctx.runQuery(
      sql = s"SELECT item, updated_at FROM $tableName ORDER BY updated_at",
      extractor = BaseHistory.tupled
    ))
  }

  def save(entity: BaseHistory): Future[Unit] = {
    val q = for {
      _ <- ctx.runAction(
