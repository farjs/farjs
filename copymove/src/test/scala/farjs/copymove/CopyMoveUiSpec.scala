...
class CopyMoveUiSpec extends AsyncTestSpec with BaseTestSpec with TestRendererUtils {
  ...
  it should "show CopyItemsStats when copy" in {
    ...
    //then
    actions.readDir.expects(currDir.path, to: js.UndefOr[String]).returning(Future.successful(toDir))
  ...
  it should "dispatch FileListParamsChangedAction if selected when onDone" in {
    ...
    val to = "test to path"
    fromActions.readDir.expects(leftDir.path, to: js.UndefOr[String]).returning(Future.successful(toDir))
