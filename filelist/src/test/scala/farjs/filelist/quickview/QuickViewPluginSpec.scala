...
class QuickViewPluginSpec extends TestSpec {
  ...
  it should "define triggerKey" in {
    //when & then
    QuickViewPlugin.triggerKey shouldBe "C-q"
  }
  ...
  it should "remove plugin from left panel when onTrigger" in {
    ...
    //when
    QuickViewPlugin.onTrigger(isRight = false, leftStack.stack, rightStack.stack)
  }
  ...
  it should "add plugin to left panel when onTrigger" in {
    ...
    //when
    QuickViewPlugin.onTrigger(isRight = true, leftStack.stack, rightStack.stack)
  }
