package farjs.ui

import scommons.react.blessed.BlessedStyle

import scala.scalajs.js

sealed trait ListViewProps extends js.Object {
  val left: Int
  val top: Int
  val width: Int
  val height: Int
  val items: js.Array[String]
  val viewport: ListViewport
  val setViewport: js.Function1[ListViewport, Unit]
  val style: BlessedStyle
  val onClick: js.Function1[Int, Unit]
}

object ListViewProps {

  def apply(left: Int,
            top: Int,
            width: Int,
            height: Int,
            items: js.Array[String],
            viewport: ListViewport,
            setViewport: js.Function1[ListViewport, Unit],
            style: BlessedStyle,
            onClick: js.Function1[Int, Unit]): ListViewProps = {

    js.Dynamic.literal(
      left = left,
      top = top,
      width = width,
      height = height,
      items = items,
      viewport = viewport.asInstanceOf[js.Any],
      setViewport = setViewport,
      style = style,
      onClick = onClick
    ).asInstanceOf[ListViewProps]
  }

  def unapply(arg: ListViewProps): Option[(Int, Int, Int, Int, js.Array[String], ListViewport, js.Function1[ListViewport, Unit], BlessedStyle, js.Function1[Int, Unit])] = {
    Some((
      arg.left,
      arg.top,
      arg.width,
      arg.height,
      arg.items,
      arg.viewport,
      arg.setViewport,
      arg.style,
      arg.onClick
    ))
  }
}
